...
public final class GridIterator<T> implements Iterator<Cell> {
    ...
    protected GridIterator(final Grid<T> grid, final Cell next, final IterDir direction) {
        this.grid = grid;
        this.next = next;
    ...
    public Stream<Cell> stream() {
        return Stream.generate(() -> null).takeWhile(x -> this.hasNext()).map(x -> this.next());
    }
}
